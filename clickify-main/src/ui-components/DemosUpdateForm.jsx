...
export default function DemosUpdateForm(props) {
  const {
    id: idProp,
    demos: demosModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = demosRecord ? { ...initialValues, ...demosRecord } : initialValues;
  ...
  const [demosRecord, setDemosRecord] = React.useState(demosModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Demos, idProp) : demosModelProp;
      setDemosRecord(record);
    };
    queryData();
  }, [idProp, demosModelProp]);
  React.useEffect(resetStateValues, [demosRecord]);
  const validations = {
    name: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Demos.copyOf(demosRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "DemosUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      <Flex justifyContent="space-between" {...getOverrideProps(overrides, "CTAFlex")}>
        <Button
          ...
          isDisabled={!(idProp || demosModelProp)}
        ...
        <Flex gap="15px" {...getOverrideProps(overrides, "RightAlignCTASubFlex")}>
          <Button
            ...
            type="submit"
            variation="primary"
            isDisabled={!(idProp || demosModelProp) || Object.values(errors).some((e) => e?.hasError)}
