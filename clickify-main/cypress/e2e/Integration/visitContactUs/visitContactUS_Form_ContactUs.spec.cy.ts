describe("Testing the form within a pop-up window", () => {
  it("Opens the pop-up window and checks the form", () => {
    ...
    cy.get(".css-1i1aivq").within(() => {
      ...
      cy.get("MuiSelect-nativeInput css-yf8vq0-MuiSelect-nativeInput").click();
      cy.get("input").eq(1).type("Blue öyster Cult");
      cy.get("input").eq(2).type("agentsof@fortune.net");
      cy.get("input").eq(3).type("don't fear the reaper");
    ...
    // TODO to finish when DB implemented
